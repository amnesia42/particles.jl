...
# update 23/10/23 it seems we need way more particles, so the other implementation is also tried here.
...
function new_gpu_compute_concentration_by_kernel(t,x_grid,Locparticles)
    ...
    numthreads = Nparticles  # This can be changed to length of a 3D grid if necessary.
    ...
    function gpu_kernel_estimator!(Kernel_Estimator, pdf_grid, x_grid, Locparticles, bandwidth)
        ...
        @inbounds CUDA.atomic_add!(pdf_grid, Kernel_Estimator(x_grid, Locparticles[i], bandwidth[1]))
        return nothing
    end
    ...
    return Array(cu_pdf_grid) ./ (Nparticles*bandwidth^1) # dimension d=1
end
...
for k=1:length(dt_list)
    ...
    for i = 1:length(t_obs)
        ti = t_obs[i]
        d = data[:, i]
        pdf_times[:, i] = new_gpu_compute_concentration_by_kernel(ti,xs,d)
    end
